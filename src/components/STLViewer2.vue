<template class="stl-viewer">
  <div class="options-div">
    <div class="select-div">
      ...
      <n-button @click="toggleExplode" tertiary v-if="currentModel && currentModel.isGroup">
        <template #icon>
          <n-icon>
...
async function loadModel() {
  ...
  if (isNewModule) {
    ...
    const fullMeshes: THREE.Object3D[] = [];
    ...
    for (const p of allModelParts) {
      ...
      if (isVRML) {
        const vrmlScene = await loadVRML(p.src);
        vrmlScene.userData.src = p.src;
        fullMeshes.push(vrmlScene);
      } else {
  ...
  currentGroup.children.forEach((mesh) => {
    if (!(mesh instanceof THREE.Mesh) && !(mesh instanceof THREE.Scene)) return;
...
function computeBaseExplosionVectors(meshes: THREE.Object3D[], referenceGroup: THREE.Group) {
